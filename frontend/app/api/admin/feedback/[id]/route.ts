...
const VALID_STATUS = ["open", "in_review", "resolved", "dismissed"] as const;
type Status = (typeof VALID_STATUS)[number];

/**
 * PATCH /api/admin/feedback/[id] → atualiza status ou admin_note
...
export async function PATCH(
  ...
  const { data, error } = await supabase
    ...
    .eq("id", id)
    .select(
      "id, type, title, description, page_url, user_agent, status, admin_note, created_at, updated_at, user_id",
    )
    .single();
...
/**
 * DELETE /api/admin/feedback/[id] → remove um report (admin-only)
 */
